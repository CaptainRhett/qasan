...
def find_files_in_dir(directory, regex, silent=True):
	...
	在特定目录中查找与正则表达式匹配的文件（非目录）。（递归，不区分大小写）
	可以传递可选参数 silent=False 来打印与正则表达式不匹配的文件名。
	"""
	matching_files = []
...
def find_testcase_functional_variants_in_dir(dir):
	...
	variant.
	查找目录中的所有功能变体。这最初是在我们决定将测试用例按功能变体拆分到单独的目录中时创建的。
	"""
...
def run_commands(commands, use_shell=True):
	...
	运行命令，就像在命令提示符中运行一样。如果您需要使用诸如“cd、dir 等”之类的命令，请将 use_shell 设置为 True。
	"""
	...
	subprocess.check_call(command, shell=use_shell, stderr=subprocess.STDOUT , stdout=f)
...
def run_analysis(test_case_path, build_file_regex):
	...
	with open("testcase_output.log", "w") as f:
	...
	# run all the files using the function pointer
		for file in files:
			...
			# print(full_path)
			command = "/home/wuhuang/fuzz/qasan/qasan-qemu -L /home/wuhuang/fuzz/qasan/cramfs-root -E LD_PRELOAD=/home/wuhuang/fuzz/qasan/libqasan/libqasan.so " + full_path
			print(command)
			try:
				sys.stdout.flush()
				subprocess.check_call(command, shell=True, stderr=subprocess.STDOUT , stdout=f)
				# time.sleep(1)
			except :
				print("\033[1;31mERROR\033[0m",command)
